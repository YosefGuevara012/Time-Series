...
names <- colnames(Canada)
cat("Variable","t test","\t", "5 pct","\n")
for(i in 1:4){
 
  test <- summary(ur.df(Canada[, i], type = "none"))
  cat( names[i],"\t", test@teststat[1],"\t")
  cat( test@cval[2], "\n")
}
...
for(i in 1:4){
  ...
  test <- summary(ur.df(Canada[, i], type = "trendte"))
  cat( names[i],"\t", test@teststat[1],"\t")
  cat( test@cval[2], "\n")
}
